...
PredictClaw 的版本以 `predict/pyproject.toml` 为唯一 source of truth。无论你是在 GitHub 上核对版本，还是准备从源码安装，都应该看 `predict/` 子目录；仓库根目录是 Foundry 工作区，不定义 PredictClaw 自己的 `pyproject.toml`。
