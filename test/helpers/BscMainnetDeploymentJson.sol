...
library BscMainnetDeploymentJson {
    using stdJson for string;

    string internal constant FILE_PATH = "deployments/bsc-mainnet.json";
    ...
    function isCompleteRaw(string memory raw) internal view returns (bool) {
        return !_isPlaceholder(raw.readStringOr(".factory", ""))
            ...
            && !_isPlaceholder(raw.readStringOr(".adapters.venus", ""))
            && !_isPlaceholder(raw.readStringOr(".adapters.pancakeswap", ""))
            && !_isPlaceholder(raw.readStringOr(".adapterCodehashes.venus", ""))
            && !_isPlaceholder(raw.readStringOr(".adapterCodehashes.pancakeswap", ""))
            && !_isPlaceholder(raw.readStringOr(".adapterLeaves.venus", ""))
            && !_isPlaceholder(raw.readStringOr(".adapterLeaves.pancakeswap", ""));
    }
